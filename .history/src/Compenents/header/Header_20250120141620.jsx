  ...
  return (
    <header
      className={`fixed w-full z-50 transition-all duration-300 ${
        isScrolled
          ? 'py-4 bg-[#1B202A]/80 backdrop-blur-md shadow-lg'
          : 'py-6 bg-transparent'
      }`}
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {menuItems.map((item, index) => (
              <a
                key={index}
                href={item.href}
                className="group relative flex items-center gap-2 text-sm md:text-base text-gray-300 hover:text-white
          ...
          {/* Navigation Mobile */}
          <nav
            className={`lg:hidden fixed top-[72px] left-0 w-full bg-[#1B202A]/95 backdrop-blur-lg
            ...
            <div className="container mx-auto px-4 py-4 space-y-4">
              {menuItems.map((item, index) => (
                <a
                  ...
                  className="group flex items-center gap-3 text-sm text-gray-300 hover:text-white
                           transition-colors duration-300 py-3 border-b border-gray-700/50
                           last:border-none"
                >
                  <span className="text-orange-400 group-hover:rotate-12 transition-transform duration-300">
                    {item.icon}
