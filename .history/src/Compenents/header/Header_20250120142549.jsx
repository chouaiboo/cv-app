...
export default function Header() {
  ...
  return (
    <header className={`fixed w-full z-50 transition-all duration-300
                     ${isScrolled 
                       ? 'py-2 sm:py-3 md:py-4 bg-[#1B202A] shadow-lg' 
                       : 'py-3 sm:py-4 md:py-6 bg-[#1B202A]/90'}`}>
            ...
            <div className="container mx-auto px-4 py-2 sm:py-3 md:py-4">
              {menuItems.map((item, index) => (
                <a
                  ...
                  className="group flex items-center gap-2 sm:gap-3 text-gray-300 hover:text-white
                           text-sm sm:text-base
                           transition-colors duration-300 
                           py-3 sm:py-4 border-b border-gray-700/50
                           last:border-none"
                >
                  <span className="text-orange-400 group-hover:rotate-12 transition-transform duration-300">
                    {item.icon}
